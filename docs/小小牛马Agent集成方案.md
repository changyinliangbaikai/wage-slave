...
> [!WARNING]
> **历史设计与现状演进说明**  
> 本文档是项目开发初期 Agent 能力的顶层设计方案，用于参考系统演进思路。
> 实际落地的系统经历了几次重大重构与深度优化，目前已与原设计案产生如下核心差异：
> 1. **定时调度架构完全解耦**：普通定时任务调度器（执行 Shell 脚本）与 Agent Cron 调度器彻底独立运行。Agent Cron 采用独立的 JSON 存储和 30 秒独立调度轮询，原设计案中二者混合调度的模型已被废弃。
> 2. **统一对话与 Chat 分流**：原设计中分裂的 `AIChat` 和 `AgentChat` 独立窗口、以及对应的 `AI_CHAT_*` 和 `AGENT_*` 通道已被全部移除。现统一使用统一对话窗口 `Chat.tsx`（对应 `CHAT_START` 统一入口通道），在主进程 `DialogueService` 内部进行普通聊天模式与 Agent 编排器模式的分流。
> 3. **失控子进程组物理强杀**：原方案对 `child_process` 的 `run_command` 控制力有限。现已实现通过 `detached: true` 开启进程组，在超时、超限或用户点击停止时，使用负 PID（macOS）或 `taskkill`（Windows 进程树）彻底物理消杀所有派生子进程（如 `sleep` 进程残留），杜绝孤儿进程与管道死锁。
> 4. **窗口生命周期自动消杀绑定**：现在已将所有活跃 Agent 状态纳入全局管理。当用户直接关闭对话窗口时，主进程会在窗口 `closed` 事件中自动触发 `abortAllActiveChats()` 批量中断执行，瞬间清理掉所有后台执行体及关联子进程，解决了原方案中直接关闭窗口导致后台长命令仍继续运行并在运行完后弹出气泡的严重 Bug。
